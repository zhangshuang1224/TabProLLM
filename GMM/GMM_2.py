...
def generate_multicolumn_gmm_prompt_and_plot(df, n_components=10, selected_cols=None):
    # 自动解析列编号为列名
    if selected_cols is None:
        ...
        numeric_cols = [
            ...
            if (isinstance(col, int) and col < len(df.columns)) or (isinstance(col, str) and col in df.columns)
        ]
    ...
    for col in numeric_cols:
        ...
        adaptive_components = min(n_components, len(np.unique(data)))
        if adaptive_components < 1:
            adaptive_components = 1

        gmm = GaussianMixture(n_components=adaptive_components, random_state=0)
        try:
            gmm.fit(data_scaled)
        except Exception as e:
            print(f"列 {col} GMM 拟合失败: {e}")
            continue
...
selected_cols = list(range(df.shape[1] - 1))
generate_multicolumn_gmm_prompt_and_plot(df, n_components=10, selected_cols=selected_cols)
